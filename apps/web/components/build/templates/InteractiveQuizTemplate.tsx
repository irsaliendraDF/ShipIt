...
export function InteractiveQuizTemplate() {
  ...
  if (done) {
    ...
    return (
      <div className="bg-cream border border-jet/10 rounded-2xl p-8 sm:p-10 text-center max-w-[560px] mx-auto">
        <p className="font-pixel text-[10px] uppercase text-purple tracking-[0.18em]">YOUR SCORE</p>
        <p className="mt-4 font-display italic text-[56px] leading-none text-jet">
          {score} <span className="text-purple">/</span> {QUESTIONS.length}
        </p>
        ...
        <div className="mt-7 flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          <button className="font-sans font-semibold text-[15px] text-jet/70 hover:text-purple transition-colors px-4 py-2.5">
            share my score →
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-cream border border-jet/10 rounded-2xl p-7 sm:p-9 max-w-[640px] mx-auto">
          ...
          return (
            <button
              ...
              className={[
                ...
                picked === null
                  ? 'border-jet/15 hover:border-purple hover:bg-purple/5'
                  : isCorrect
                    ? 'border-purple bg-purple/10 text-jet'
                    : isWrong
                      ? 'border-orange bg-orange/10 text-jet'
            ...
            >
              ...
              {opt}
              {isCorrect && <span className="ml-2 text-purple">✓</span>}
              {isWrong && <span className="ml-2 text-orange">✗</span>}
            </button>
          );
        })}
      </div>
