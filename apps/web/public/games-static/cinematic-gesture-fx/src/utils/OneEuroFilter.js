// One-Euro filter for landmark smoothing. Low lag, kills jitter.
// Reference: Casiez et al. 2012. Separate filter per signal channel.
//
// Usage: `const f = new OneEuroFilter({minCutoff: 1.5, beta: 0.05});
//         const smoothed = f.filter(x, timestampSec);`
//
// For 21 landmarks × 3 coords we use a HandSmoother wrapper that holds
// 63 independent filters and resets them when a hand disappears.
...
// Per-hand wrapper: 63 filters for landmarks x/y/z. reset() when hand lost.
export class HandSmoother {
  ...
  // landmarks: array of {x,y,z} (21). Returns a NEW array with smoothed values.
